...
function resetForm() {
	// Reset personal
	Object.keys(formData.personal).forEach(
...
async function loadProfileData(id) {
	try {
		...
		if (data.personal.house_number)
			formData.personal.housenumber = data.personal.house_number;
		if (data.personal.postal_code)
			formData.personal.postalcode = data.personal.postal_code;
		if (data.personal.blood_type)
			formData.personal.bloodtype = data.personal.blood_type;
		...
		if (data.personal.avatar_url)
			formData.personal.avatar_url = data.personal.avatar_url;
		...
		// Map backend snake_case to frontend camelCase/simple
		if (data.doctor.house_number)
			formData.doctor.housenumber = data.doctor.house_number;
		if (data.doctor.postal_code)
			formData.doctor.postalcode = data.doctor.postal_code;
		...
		loadedIds.med_id =
			data.medical.med_id || data.medical.medications?.[0]?.med_id || null;
	} catch (e) {
...
async function saveProfile() {
	try {
		...
		if (
			!formData.personal.avatar_url ||
			formData.personal.avatar_url.trim() === ""
		) {
			formData.personal.avatar_url =
				"https://img.freepik.com/vektoren-premium/vektorflat-illustration-in-grauen-farben-avatar-benutzerprofil-person-ikonen-geschlechtsneutrale-silhouette-profilbild-geeignet-fuer-social-media-profilen-ikonen-bildschirmschutz-und-als-vorlagex9_719432-848.jpg";
		}
		...
		if (isEditMode.value && loadedIds.profile_id) {
			...
			await userStore.updatePersonal(
				loadedIds.profile_id,
				personalUpdatePayload,
			);
			...
			for (const contact of emergencyContactsData) {
				if (contact.contact_id) {
					...
					await userStore.saveEmergencyContacts(loadedIds.profile_id, [
						contact,
					]);
				}
			}
			...
			if (loadedIds.med_id) {
				await userStore.updateMeddata(loadedIds.med_id, {
					key_info: medicalData.key_info,
				});
				...
				const saveMedical = await userStore.saveMeddata(
					loadedIds.profile_id,
					medicalData,
				);
			...
			const savePersonal = await userStore.savePersonal(
				props.userId,
				personalData,
			);

			if (savePersonal && savePersonal.profile_id) {
				await userStore.saveDoctor(savePersonal.profile_id, doctorData);

				if (
					formData.emergencyContacts &&
					formData.emergencyContacts.length > 0
				) {
					await userStore.saveEmergencyContacts(
						savePersonal.profile_id,
